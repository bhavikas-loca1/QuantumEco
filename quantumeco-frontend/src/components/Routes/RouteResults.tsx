...
const RouteResults: React.FC<RouteResultsProps> = ({
  ...
  const getLocationName = (locationRef: any) => {
  // Handle both index numbers and location objects
  if (typeof locationRef === 'number') {
    // Location index
    if (locations[locationRef]) {
      return locations[locationRef].name || locations[locationRef].address || `Location ${locationRef}`;
    }
    return `Location ${locationRef}`;
  } else if (typeof locationRef === 'object' && locationRef !== null) {
    // Location object
    return locationRef.name || locationRef.address || locationRef.id || 'Unknown Location';
  }
  return `Location ${locationRef}`;
};
...
const getOptimizedRoutes = (): OptimizedRoute[] => {
  if (optimizationResult?.optimized_routes) return optimizationResult.optimized_routes;
  if (comparisonResult?.quantum_inspired_result?.routes) return comparisonResult.quantum_inspired_result.routes;
  return [];
};
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
      ...
      // ✅ Fixed: Update the comparison results display
      {comparisonResult && (
        <Card>
          <CardContent>
                  ...
                  <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                      ...
                      <Typography variant="body2" sx={{ fontWeight: 'bold', color: 'success.main' }}>
                        {safeToFixed(comparisonResult.improvements.cost_improvement, 2)}%
                      </Typography>
                    </Box>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                      <Typography variant="body2" color="text.secondary">Carbon Reduction:</Typography>
                      <Typography variant="body2" sx={{ fontWeight: 'bold', color: 'success.main' }}>
                        {safeToFixed(comparisonResult.improvements.carbon_improvement, 2)}%
                      </Typography>
                    </Box>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                      <Typography variant="body2" color="text.secondary">Time Reduction:</Typography>
                      <Typography variant="body2" sx={{ fontWeight: 'bold', color: 'success.main' }}>
                        {safeToFixed(comparisonResult.improvements.time_improvement, 2)}%
                      </Typography>
                    </Box>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                      <Typography variant="body2" color="text.secondary">Distance Saved:</Typography>
                      <Typography variant="body2" sx={{ fontWeight: 'bold', color: 'success.main' }}>
                        {safeToFixed(comparisonResult.improvements.distance_improvement, 2)}%
                      </Typography>
                    </Box>
                    <Divider sx={{ my: 1 }} />
                    <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                      ...
                      <Chip 
                        label={`${safeToFixed((comparisonResult.improvements.cost_improvement + comparisonResult.improvements.time_improvement + comparisonResult.improvements.carbon_improvement) / 3, 1)}% better`} 
                        size="small" 
                        color="success"
                        icon={<TrendingUp />}
                      />
                      ...
                      <TableRow key={route.vehicle_id || index}>
                        ...
                        <TableCell>
                          <Box sx={{ maxWidth: 200 }}>
                            ...
                            >
                              {route.locations ? `${route.locations.length} stops` : 'View Route'}
                            </Button>
                              ...
                              <Box sx={{ mt: 1, pl: 1, borderLeft: '2px solid', borderColor: 'primary.main' }}>
                                {route.locations?.map((location, idx) => (
                                  <Typography key={idx} variant="caption" display="block" color="text.secondary">
                                    {idx + 1}. {location.name || location.address || `Location ${idx + 1}`}
                                  </Typography>
                                ))}
                              </Box>
                            </Collapse>
                        ...
                        {/* ✅ FIXED: Use correct field names and safe defaults */}
                        <TableCell align="right">{formatDistance(route.distance_km || 0)}</TableCell>
                        <TableCell align="right">{formatDuration(route.time_minutes || 0)}</TableCell>
                        <TableCell align="right">{formatCurrency(route.cost_usd || 0)}</TableCell>
                        <TableCell align="right">{safeToFixed(route.carbon_kg || 0, 1)} kg</TableCell>
                        ...
                        <TableCell align="right">
                          <Box>
                            <LinearProgress
                              variant="determinate"
                              value={route.utilization_percent || 0}
                              sx={{ mb: 0.5, height: 4 }}
                              color={
                                (route.utilization_percent || 0) > 90 ? 'error' : 
                                (route.utilization_percent || 0) > 70 ? 'warning' : 'success'
                              }
                            />
                            <Typography variant="caption" color="text.secondary">
                              {safeToFixed(route.utilization_percent || 0, 0)}%
                            </Typography>
                          </Box>
                        </TableCell>
                      </TableRow>
      ...
      {(optimizationResult || comparisonResult) && (
        <Alert severity="success" sx={{ mt: 2 }}>
          ...
          <Typography variant="body2">
            ...
            {comparisonResult && (
              <>Method comparison shows quantum-inspired routing outperformed traditional methods by large margins. </>
            )}
